...
In the \nameref{sec:background:hardware:spi} (\ref{sec:background:hardware:spi}) and \nameref{sec:background:hardware:ic-mu} (\ref{sec:background:hardware:ic-mu}) sections we have already seen how SPI and the position encoder that we use work.
...
In fact in the \nameref{sec:concept_and_implementation:linux:rppal} (\ref{sec:concept_and_implementation:linux:rppal}) chapter,
...
As we have seen in the \nameref{sec:background:rust:embedded} (\ref{sec:background:rust:embedded}) section the traditional way to program for a microcontroller in Rust
...
First, we will add a .cargo/config.toml similar to what we saw in \nameref{sec:concept_and_implementation:bare-metal:cross} (\ref{sec:concept_and_implementation:bare-metal:cross}).
\begin{lstlisting}
[target.aarch64-unknown-none]
linker = "aarch64-none-elf-ld"
...
Both are very similar to what we already saw in the \nameref{sec:concept_and_implementation:bare-metal:boot} (\ref{sec:concept_and_implementation:bare-metal:boot}) chapter, so we will show only the differences here.
...
Building the final binary works the same as in \nameref{sec:concept_and_implementation:bare-metal:boot} (\ref{sec:concept_and_implementation:bare-metal:boot}).
...
To measure the iteration-time deltas, we use Circle's CTimer class. The accuracy of this clock will be discussed in more detail in the \nameref{sec:experiments:methodology} (\ref{sec:experiments:methodology}) section.
